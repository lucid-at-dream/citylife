        ...
        for (int i = 0; i < argc; i++) {
                ...
                if (strcmp(desc, _config_cfg_file_arg_desc.short_name) == 0 ||
                    strcmp(desc, _config_cfg_file_arg_desc.long_name) == 0) {
                        if (argc <= i + 1) {
                                elegant_exit(arg_desc_count, arg_desc, args,
                                             "No path given for the configuration file.");
                        }
                        parse_config_file(args, argv[i + 1], arg_desc_count, arg_desc);
                }

                if (strcmp(desc, _config_help_arg_desc.short_name) == 0 ||
                    strcmp(desc, _config_help_arg_desc.long_name) == 0) {
                        print_usage(arg_desc_count, arg_desc);
                }
        }
