        ...
        if (this->type != GRAY) {
                if (this->dictionary != NULL) {
                        for (list<GIMS_Geometry *>::iterator it = dictionary->begin(); it != dictionary->end(); it++) {
                                if (filter(*it)) {
                                        matrix_t::iterator m_it = resultset->setIntersect(
                                                (*it)->id,
                    ...
                    query's interior.*/
                      ...
                      contained border is solely a polygon's interior ring, *it's exterior intersects query's border*/
                                        if ( //if it is not a polygon
                                                ...
                                                || ((*it)->type == POLYGON &&
                                                    (((GIMS_Polygon *)(*it))->externalRing != NULL &&
...
void Node::activeSearch(DE9IM *resultset, GIMS_Geometry *query, int (*filter)(GIMS_Geometry *))
{
        ...
        if (this->type == GRAY) {
                ...
                for (list<GIMS_Geometry *>::iterator it = this->dictionary->begin(); it != this->dictionary->end();
                     it++) {
...
DE9IM *PMQuadTree::topologicalSearch(GIMS_Geometry *query, int (*filter)(GIMS_Geometry *))
{
        ...
        if (__PMQT__verifyPolygonContainment__) {
                ...
                case LINESTRING:
                case RING:
                        pt = ((GIMS_LineString *)query)->list[0];
                        break;
                case POLYGON:
                        pt = ((GIMS_Polygon *)query)->externalRing->list[0]->list[0];
                        break;
                case MULTILINESTRING:
                        pt = ((GIMS_MultiLineString *)query)->list[0]->list[0];
                        break;
                case MULTIPOLYGON:
                        pt = ((GIMS_MultiPolygon *)query)->list[0]->externalRing->list[0]->list[0];
                        break;
                default:
                ...
                while ((n = n->goNorth(pt->x)) != NULL) {
                        ...
                        for (list<GIMS_Geometry *>::iterator it = n->dictionary->begin(); it != n->dictionary->end();
                             it++) {
...
void PMQuadTree::debugRender(Cairo::RefPtr<Cairo::Context> cr)
{
        ...
        if (blackRenderQueue->size() > 0) {
                for (list<GIMS_Geometry *>::iterator it = blackRenderQueue->begin(); it != blackRenderQueue->end();
                     it++) {
                        cr->stroke();
