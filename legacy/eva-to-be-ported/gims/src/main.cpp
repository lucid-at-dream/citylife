...
int main(int argc, char **argv)
{
        ...
        list<GIMS_Geometry *> planet_osm_point =
                loadPostgisData("FROM planet_osm_point ORDER BY osm_id DESC LIMIT 500");
        ...
        list<GIMS_Geometry *> planet_osm_polygon =
                loadPostgisData("FROM planet_osm_polygon ORDER BY osm_id ASC LIMIT 25");
        ...
        long long int id;
        char buffer[254];
        int queryidslimit = 10000;

        f = fopen("sqldata/idlists/pt", "r");
        list<GIMS_Geometry *> query_points;
        while (fscanf(f, "%lld", &id) != EOF) {
                for (list<GIMS_Geometry *>::iterator it = planet_osm_point.begin(); it != planet_osm_point.end();
                     it++) {
                        if ((*it)->osm_id == id) {
        ...
        while (fscanf(f, "%lld", &id) != EOF) {
                for (list<GIMS_Geometry *>::iterator it = planet_osm_polygon.begin(); it != planet_osm_polygon.end();
                     it++) {
                        if ((*it)->osm_id == id) {
