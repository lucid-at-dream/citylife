...
int reduceHull(GIMS_Point **ch_points, int N)
{
        ...
        for (int i = 0; i < N; i++) {
                ...
                GIMS_Point *A_p1 = ch_points[mod(i - 1, N)], *A_p2 = ch_points[mod(i, N)],
                           *B_p2 = ch_points[mod(i + 1, N)], *B_p1 = ch_points[mod(i + 2, N)];
                ...
                if (s > 1 && t > 1) {
                        ...
                        double area = fabs(p1->x * p2->y - p2->x * p1->y + p2->x * p3->y - p3->x * p2->y +
                                           p3->x * p1->y - p1->x * p3->y) /
                                      2.0;
