...
int reduceHull(GIMS_Point **ch_points, int N){
    ...
    for( int i=0; i<N; i++ ){
        ...
        GIMS_Point *A_p1 = ch_points[mod(i - 1,N)],
                   *A_p2 = ch_points[mod(i    ,N)],
                   *B_p2 = ch_points[mod(i + 1,N)],
                   *B_p1 = ch_points[mod(i + 2,N)];

        mpf_class A_p1_x = A_p1->x,
                  A_p1_y = A_p1->y,
                  A_p2_x = A_p2->x,
                  A_p2_y = A_p2->y,
                  B_p1_x = B_p1->x,
        ...
        mpf_class s_den = (-s2_x * s1_y + s1_x * s2_y), 
                 s_num = (-s1_y * (A_p1_x - B_p1_x) + s1_x * (A_p1_y - B_p1_y)),
                 t_den = (-s2_x * s1_y + s1_x * s2_y),
                 t_num = ( s2_x * (A_p1_y - B_p1_y) - s2_y * (A_p1_x - B_p1_x));

        /*if the lines are parallel ignore.*/
        if( (s_den < ERR_MARGIN && s_den > -ERR_MARGIN) || 
            (t_den < ERR_MARGIN && t_den > -ERR_MARGIN) )
            continue;
        ...
        if( s > 1 && t > 1 ){
            mpf_class tmp_x = A_p1_x + (t * s1_x),
                      tmp_y = A_p1_y + (t * s1_y);

            double int_x = tmp_x.get_d(),
                   int_y = tmp_y.get_d();
            ...
            double area = fabs( p1->x * p2->y - p2->x * p1->y +
                                p2->x * p3->y - p3->x * p2->y +
