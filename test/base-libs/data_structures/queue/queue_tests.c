...
char test_queue_add_pop()
{
        ...
        for (int i = 0; i < length; i++) {
                if (ops[i].type == 1) {
                        ...
                        int assertion_error = assert_str_equals(
                                "The item that popped from the queue should be the expected one.", data, ops[i].data);
                        if (assertion_error) {
                                return 1;
                        }
                }

                int assertion_error = assert_int_equals("The number of items in queue matches what is expected",
                                                        q->size, ops[i].size);
                if (assertion_error) {
                        return 1;
                }
